...
As all variables but year were heavily right skewed, log transformations were taken for all predictors (except for year). Because all pesticides had a high number of 0’s, we added 1 to each of these observations so that it still remained 0 after we took the log. See appendix for further discussion of this.  
...
Interestingly, we didn’t find that most of the neonic chemicals were significant in our model. We saw a significant interaction between kilograms of clothianidin and acetamiprid, although neither chemical on its own proved significant. This is seen in the confidence intervals for each coefficient. The confidence interval for both log(clothianidin) and log(acetamiprid) contain 0; however, their interaction has a negative effect on the log of total production of honey. 
